...
function update_ψ_x!(ψ_x_l, ψ_x_r, pcur,
                     halo, _dx, nx,
                     a_x_hl, a_x_hr,
                     b_K_x_hl, b_K_x_hr)
...
function update_ψ_y!(ψ_y_l, ψ_y_r, pcur,
                     halo, _dy, ny,
                     a_y_hl, a_y_hr,
                     b_K_y_hl, b_K_y_hr)
...
function update_ψ_z!(ψ_z_l, ψ_z_r, pcur,
                     halo, _dz, nz,
                     a_z_hl, a_z_hr,
                     b_K_z_hl, b_K_z_hr)
    ...
    for k=2:nz-1
        for j=2:ny-1
            for i=2:nx-1
                ...
                # update pressure
                pnew[i,j,k] = 2.0 * pcur[i,j,k] - pold[i,j,k] +
                    fact[i,j,k] * (d2p_dx2 + d2p_dy2 + d2p_dz2) + damp
...
function correlate_gradient!(curgrad, adjcur, pcur, pold, pveryold, dt)
    ...
    for k=1:nz
        for j=1:ny
            for i=1:nx
                curgrad[i,j,k] = curgrad[i,j,k] + ( adjcur[i,j,k] * ( pcur[i,j,k] - 2.0 * pold[i,j,k] + pveryold[i,j,k] ) * _dt2 )
            end
        end
    end
    return nothing
