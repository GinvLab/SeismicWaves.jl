@parallel_indices (i, j) function correlate_gradient_ρ_kernel!(grad_ρ, adjcur, uold, ucur, unew, _dt2, freeboundtop)
    if j == 1 && freeboundtop
        # Half contribution at free surface
        grad_ρ[i, j] += (adjcur[i, j] * (uold[i, j] - 2 * ucur[i, j] + unew[i, j]) * _dt2) / 2
    else
        grad_ρ[i, j] += (adjcur[i, j] * (uold[i, j] - 2 * ucur[i, j] + unew[i, j]) * _dt2)
    end
...
@parallel_indices (i, j) function correlate_gradient_λ_μ_kernel!(grad_λ, grad_μ, adjux, adjuz, ux, uz, λ, μ, _Δx, _Δz, nx, nz, freeboundtop)
    ...
    if j == 1 && freeboundtop # Half contribution at free surface
        grad_λ[i, j] += div_u * div_u_adj / 2
        grad_μ[i, j] += (ε_xx * ε_xx_adj + ε_zz * ε_zz_adj)
    else
...
function correlate_gradients!(grid, uold_corr, ucur_corr, unew_corr, dt, freeboundtop)
    nx, nz = grid.size
    @parallel (1:nx-1, 1:nz) correlate_gradient_ρ_kernel!(
    ...
    @parallel (1:nx, 1:nz-1) correlate_gradient_ρ_kernel!(
