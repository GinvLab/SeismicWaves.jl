...
@parallel function update_psi_CPML_parstencil!(nx::Data.Number,nz::Data.number,pcur::Data.Array,
                                               dpdx::Data.Array,dpdz::Data.Array,
                                               psi_x::Data.Array,psi_z::Data.Array  )
    ...
    dpdx[inn1,inn1] = pcur[inn1p,inn1]-pcur[inn1,inn1] 
    dpdz[inn1,inn1] = pcur[inn1,inn1p]-pcur[inn1,inn1]
    psi_x[inn1,inn1] = b_x_half[inn1] / K_x_half[inn1,inn1] * psi_x[inn1,inn1] + a_x_half[inn1,inn1] * dpdx[inn1,inn1]
    psi_z[inn1,inn1] = b_z_half[inn1,inn1] / K_z_half[inn1,inn1] * psi_z[inn1,inn1] + a_z_half[inn1,inn1] * dpdz[inn1,inn1]
...
@parallel function update_p_CPML_parstencil!(nx::Data.Number,nz::Data.number,fact::Data.Array,pnew::Data.Array,
                                             pold::Data.Array,pcur::Data.Array,
                                             dt2srctf::Data.Array,
                                             dpdx::Data.Array,dpdz::Data.Array,
                                             d2pdx2::Data.Array,d2pdz2::Data.Array,
                                             psi_x::Data.Array,psi_z::Data.Array,
                                             xi_x::Data.Array,xi_z::Data.Array,
                                             # cpml::CoefPML,
                                             K_x_half::Data.Array,a_x_half::Data.Array,K_z_half::Data.Array,a_z_half::Data.Array,
                                             ijsrcs::Data.Array,t::Data.Number)
    ...
    d2pdx2[inn1,inn1] = pcur[inn1p,inn1]-2.0*pcur[inn1,inn1]+pcur[inn1m,inn1]            
    d2pdz2[inn1,inn1] = pcur[inn1,inn1p]-2.0*pcur[inn1,inn1]+pcur[inn1,inn1m]

    dpsidx[inn1,inn1] = psi_x[inn1,inn1] - psi_x[inn1m,inn1]            
    dpsidz[inn1,inn1] = psi_z[inn1,inn1] - psi_z[inn1,inn1m]            

    xi_x[inn1,inn1] = b_x[inn1] / K_x_half[inn1,inn1] * xi_x[inn1,inn1] + a_x[inn1,inn1] * (d2pdx2[inn1,inn1] + dpsidx[inn1,inn1])
    xi_z[inn1,inn1] = b_z[inn1] / K_z_half[inn1,inn1] * xi_z[inn1,inn1] + a_z[inn1,inn1] * (d2pdz2[inn1,inn1] + dpsidz[inn1,inn1])
            
    damp[inn1,inn1] = fact[inn1,inn1] * (dpsidx[inn1,inn1] + dpsidz[inn1,inn1] + xi_x[inn1,inn1] + xi_z[inn1,inn1])

    # update pressure
    pnew[inn1,inn1] = 2.0*pcur[inn1,inn1] - pold[inn1,inn1] +
        fact[inn1,inn1]*(d2pdx2[inn1,inn1] + d2pdz2[inn1,inn1]) + damp[inn1,inn1]
    ...
    isrc = ijsrcs[:,1]
    jsrc = ijsrcs[:,2]
    pnew[isrc,jsrc] = pnew[isrc,jsrc] + dt2srctf[t,:] 
...
"""
  ...
  Serial version.
"""
function solveacoustic2D_parstencil(inpar::InpParamAcou,ijsrcs::Array{Array{Int64,2},1},
    ...
    for s=1:nshots
        ...
        if inpar.boundcond=="PML"
            ...
            #### Re-creating arrays at every shot...
            #### To be optimized!
            K_x_half = @zeros(nx,nz)
            a_x_half = @zeros(nx,nz)
            K_z_half = @zeros(nx,nz)
            a_z_half = @zeros(nx,nz)
            
            K_x_half .= Data.Array(repeat(cpml.K_x_half,1,ny))
            a_x_half .= Data.Array(repeat(cpml.a_x_half,1,ny))
            K_z_half .= Data.Array(repeat(cpml.K_z_half,nx,1))
            a_z_half .= Data.Array(repeat(cpml.a_z_half,nx,1))
...
##----------------
        ...
        for t=1:inpar.ntimesteps
            ...
            if inpar.boundcond=="PML"
                ...
                @parallel oneiter_CPML!parstencil(nx,nz,fact,pnew,pold,pcur,dt2srctf,
                                                  dpdx,dpdz,d2pdx2,d2pdz2,
                                                  psi_x,psi_z,xi_x,xi_z,
                                                  #cpml,
                                                  K_x_half,a_x_half,K_z_half,a_z_half,
                                                  ijsrcs[s],t)
                ...
                error("Only PML boundary conditions allowed.")
            ...
            if (inpar.savesnapshot==true) & (t%inpar.snapevery==0) 
                isnap+=1
                psave[:,:,isnap,s] = Array(pcur) # Array(...)
            end
