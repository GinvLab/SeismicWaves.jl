...
function groupshot_acoustic2D(inpar::InpParamAcou,ijsrcs::Array{Array{Int64,2},1},
    ...
    for s=1:nshots
        ...
        elseif inpar.boundcond=="GauTap"
            ...
            error("Wrong boundary condition.")
        end
...
##----------------
        ...
        for t=1:inpar.ntimesteps
            ...
            elseif inpar.boundcond=="GauTap"
                pold,pcur,pnew = oneiter_GAUSSTAP!(nx,nz,fact,pnew,pold,pcur,dt2srctf,
                                                   ijsrcs[s],t,gaubc)
...
function gradacoustic2D_parallel(inpar::InpParamAcou, obsrecv::Array{Array{Float64,2},1},
    ...
    if verbose>0
        t2 = time()
        println("## Total gradient calculation time for all shots: ",t2-tstart)
    end
...
function groupshot_gradacou2D(inpar::InpParamAcou, obsrecv::Array{Array{Float64,2},1},
    ...
    if totmem>maxMEM
        ...
        println(" To allow higher threshold change the parameter 'maxMEM' in function groupshot_gradacou2D()")
...
##----------------
        ...
        for t=1:inpar.ntimesteps + 1 ## + 1 !!
            # if verbose>0
            #     inpar.infoevery==0 && print("\rShot ",s," t: ",t," of ",inpar.ntimesteps)
            # end
        ...
        for t = 1:nt
            ...
            elseif inpar.boundcond=="GauTap"
                ...
                adjold,adjcur,adjnew = oneiter_GAUSSTAP!(nx,nz,fact,adjnew,adjold,adjcur,
                                                         dt2srctf,thishotijsrcs_adj,t,gaubc)
        ...
        ##-----------------------------------------
        #### smooth the gradient at the source
        if inpar.smoothgrad
            smoothgradient1shot!(curgrad,ijsrcs[s],radiuspx=5)
        end
        ##-----------------------------------------
        ...
        if verbose>0
            t9=time()
            println(" Total forward solver time for 1 shot: ",t2-t0)
            println(" Total residual calculation time for 1 shot: ",t3-t0)
            println(" Total adjoint solver time for 1 shot: ",t9-t3)
        end
