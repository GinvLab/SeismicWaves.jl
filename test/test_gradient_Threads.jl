...
with_logger(error_logger) do
    @testset "Test 1D gradient! checkpointing" begin
        ...
        grad = gradient!(params, vel, [srcs => recs]; use_GPU=false, check_freq=nothing)
        # Solve gradient with (optimal) checkpointing
        grad_check = gradient!(params, vel, [srcs => recs]; use_GPU=false, check_freq=floor(Int, sqrt(nt)))
