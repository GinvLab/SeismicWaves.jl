...
with_logger(error_logger) do
    ...
    for parall in test_backends
        ...
        @testset "Test 1D $(parall) swgradient! checkpointing" begin
            ...
            grad = swgradient!(params, vel, [srcs => recs]; parall=parall, check_freq=nothing)
            # Solve gradient with (optimal) checkpointing
            grad_check = swgradient!(params, vel, [srcs => recs]; parall=parall, check_freq=floor(Int, sqrt(nt)))
