...
function exeelanoise(dt, nt, f0, t0)
    ...
    srcs = PSDMomentTensorSources(possrcs, srcstf, f0,
        [MomentTensor2D(; Mxx=1.0, Mzz=2.0, Mxz=0.5)])
    ...
    recs = VectorCrossCorrelationsReceivers(posrecs, nt, [2]) # ref rec is #2
    ...
    @show size(shots[1].recs.crosscorrelations)

    return shots[1].recs.crosscorrelations
end
